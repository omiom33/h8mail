...
class intelx:
	...
	def FILE_TREE_VIEW(self, sid):
		...
		try:
			...
			return False if "Could not generate" in r.text else r.text
		except:
			return False
	...
	def INTEL_SEARCH(self, term, maxresults=100, buckets=[], timeout=5, datefrom="", dateto="", sort=4, media=0, terminate=[]):
		...
		r = requests.post(f'{self.API_ROOT}/intelligent/search', headers=h, json=p)
	...
	def INTEL_SEARCH_RESULT(self, id, limit):
		...
		h = {'x-key' : self.API_KEY, 'User-Agent': self.USER_AGENT}
		r = requests.get(
			f'{self.API_ROOT}/intelligent/search/result?id={id}&limit={limit}',
			headers=h,
		)
	...
	def INTEL_TERMINATE_SEARCH(self, uuid):
		...
		h = {'x-key' : self.API_KEY, 'User-Agent': self.USER_AGENT}
		r = requests.get(
			f'{self.API_ROOT}/intelligent/search/terminate?id={uuid}', headers=h
		)
	...
	def PHONEBOOK_SEARCH(self, term, maxresults=100, buckets=[], timeout=5, datefrom="", dateto="", sort=4, media=0, terminate=[], target=0):
		...
		r = requests.post(f'{self.API_ROOT}/phonebook/search', headers=h, json=p)
		return r.json()['id'] if r.status_code == 200 else r.status_code
	...
	def PHONEBOOK_SEARCH_RESULT(self, id, limit=1000, offset=-1):
		...
		h = {'x-key' : self.API_KEY, 'User-Agent': self.USER_AGENT}
		r = requests.get(
			f'{self.API_ROOT}/phonebook/search/result?id={id}&limit={limit}&offset={offset}',
			headers=h,
		)
		return r.json() if (r.status_code == 200) else r.status_code
