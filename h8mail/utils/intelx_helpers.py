...
def intelx_getsearch(target, intelx, maxfile):
    ...
    c.info_news(f"[{target}]>[intelx.io] Starting search")
    ...
    available_buckets = [
        b for b in cap["buckets"] if any(b in d for d in desired_buckets)
    ]
    c.info_news(f"[{target}]>[intelx.io] Available buckets for h8mail:")
    print(available_buckets)
    c.info_news(
        f"[{target}]>[intelx.io] Search in progress (max results : {str(maxfile)})"
    )
    ...
    c.good_news(f"[{target}]>[intelx.io] Search returned the following files :")
